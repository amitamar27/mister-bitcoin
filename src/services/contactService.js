...
function getNextContactById(currContactId) {
    const currContactIdx = gContacts.findIndex(contact => contact._id === currContactId)
    const nextContactId = gContacts[currContactIdx + 1]._id
    return Promise.resolve( nextContactId )
...
function _updateContact(contact) {
    return new Promise((resolve, reject) => {
        ...
        gContacts.splice(index, 1, contact)
        storageService.store(STORAGE_KEY, gContacts)
        resolve(contact)
    })
}
